...
// Initialize the mapping section when the "Fields Mapping" section is activated
document.addEventListener('DOMContentLoaded', async () => {
    try {
        // Load example files into InputFrame and OutputFrame
        await inputFrame.loadFromFile(new File([], 'example_input.xlsx')); // Replace with actual file loading logic
        await outputFrame.loadFromFile(new File([], 'example_output.xlsx')); // Replace with actual file loading logic
        ...
        navItems.forEach(navItem => {
            navItem.addEventListener('click', () => {
                ...
                // Check if the clicked section is "fields-mapping"
                if (sectionId === 'mapping' && !isMappingInitialized) {
                    renderMappingTable();
                    isMappingInitialized = true; // Mark the mapping table as initialized
                }
            });
        });
    } catch (error) {
        console.error("Error initializing mapping section:", error);
        createNotification("Error loading headers for mapping.");
    }
});